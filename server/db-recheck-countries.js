...
const candidateSql = checkAll
  ? `SELECT m.id, m.tmdb_id FROM movies m
      WHERE COALESCE(m.is_indian, 1) = 0 AND m.tmdb_id IS NOT NULL AND m.id > ?
      ORDER BY m.id LIMIT ?`
  : `SELECT m.id, m.tmdb_id FROM movies m
      WHERE COALESCE(m.is_indian, 1) = 0 AND m.tmdb_id IS NOT NULL AND m.id > ?
        AND (SELECT COUNT(*) FROM movie_cast mc
              WHERE mc.movie_id = m.id
                AND mc.person_id IN (
                  SELECT DISTINCT mc2.person_id FROM movie_cast mc2
                    JOIN movies m2 ON m2.id = mc2.movie_id AND COALESCE(m2.is_indian, 1) = 1)
            ) >= ${Number.isFinite(minShared) ? minShared : 1}
      ORDER BY m.id LIMIT ?`;
...
if (!apply) {
  const n = db
    .prepare(
      checkAll
        ? `SELECT COUNT(*) n FROM movies m WHERE COALESCE(m.is_indian,1)=0 AND m.tmdb_id IS NOT NULL`
...
if (afterId) console.log(`\nResuming after id ${afterId}`);
